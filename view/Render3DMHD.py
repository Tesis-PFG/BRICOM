...
class MHD_3DRenderer:
    def __init__(self, mhd_file, structure):
        """
        Inicializa la clase, carga el archivo .mhd y muestra la estructura seleccionada.
        
        :param mhd_file: Ruta del archivo .mhd
        :param structure: Estructura a mostrar (1, 2, 3, etc.)
        """
        self.mhd_file = mhd_file
        self.structure = structure
        self.image_reader = self.load_mhd_image(mhd_file)

    def load_mhd_image(self, mhd_file):
        reader = vtk.vtkMetaImageReader()
        reader.SetFileName(mhd_file)
        reader.Update()
        return reader

    def extract_structure(self, image_reader, structure):
    ...
    def create_color_transfer_function(self, structure):
    ...
    def create_opacity_transfer_function(self, structure):
    ...
    def create_render_window(self, volume_mapper, color_function, opacity_function):
    ...
    def render_mhd_structure(self, structure):
        ...
        color_function = self.create_color_transfer_function(structure)
        opacity_function = self.create_opacity_transfer_function(structure)

        render_window, interactor = self.create_render_window(volume_mapper, color_function, opacity_function)
